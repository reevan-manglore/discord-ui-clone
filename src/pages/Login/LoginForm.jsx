...
export default function LoginForm() {
    ...
    return (
        <section className="p-4 bg-discord-grey rounded-md w-1/2 flex flex-none justify-between gap-x-4">
            <form className=" w-3/5 " onSubmit={handleSubmit}>
                ...
                <div className='flex flex-col gap-y-3  '>
                    <div className='flex flex-col gap-y-1 '>
                        ...
                        <input type="email" id="email" ref={email} required  pattern = "^[\w-\.]+@([\w-]+\.)+[\w-]{2,4}$" className='bg-[#2f3136] border-2 border-[#23272a] rounded-sm text-white text-lg px-4 py-0.5 focus:border-primary outline-none' />
                    </div>
                    <div className='flex flex-col gap-y-1'>
                        <label htmlFor="password" className=' text-slate-400'>Password</label>
                        <input type="password" id="password" ref={password}  required minLength={6} className='bg-[#2f3136] border-2 border-[#23272a] rounded-sm text-white text-lg px-4 py-0.5 focus:border-primary outline-none' />
                    </div>
                </div>
